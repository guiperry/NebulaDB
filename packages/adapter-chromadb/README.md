# @nebula-db/adapter-chromadb

A ChromaDB adapter for NebulaDB that enables persistent storage using ChromaDB as a document database. This adapter leverages ChromaDB's collection-based storage without utilizing its vector search capabilities.
...
```bash
npm install @nebula-db/core @nebula-db/adapter-chromadb
```
...
import { ChromadbAdapter } from '@nebula-db/adapter-chromadb';
...
Loads all data from ChromaDB collections and returns it in NebulaDB format.
...
Saves NebulaDB data to ChromaDB collections. Creates collections as needed and replaces existing data.
...
- **Collections:** Each NebulaDB collection maps to a ChromaDB collection.
