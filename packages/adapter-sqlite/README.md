...
An SQLite adapter for NebulaDB that provides persistent file-based storage using SQLite's embedded database capabilities.
...
```bash
npm install @nebula-db/core @nebula-db/adapter-sqlite
```
...
Saves NebulaDB data to the SQLite database using transactions for atomicity.
