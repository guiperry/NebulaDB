# @nebula-db/adapter-filesystemdb

A filesystem adapter for NebulaDB that provides persistent storage using JSON files on disk. This adapter uses the `file-system-db` package for efficient, synchronous file operations.
...
```bash
npm install @nebula-db/core @nebula-db/adapter-filesystemdb
```
...
import { FilesystemAdapter } from '@nebula-db/adapter-filesystemdb';
...
Saves NebulaDB data to the filesystem database file.
