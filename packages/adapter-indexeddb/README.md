...
An IndexedDB adapter for NebulaDB that provides persistent client-side storage in web browsers. This adapter uses the browser's native IndexedDB API for efficient, asynchronous data storage.
...
```bash
npm install @nebula-db/core @nebula-db/adapter-indexeddb
```
...
Saves NebulaDB data to the IndexedDB database.
...
- **Database:** Uses IndexedDB database named `'NebulaDB'`.
