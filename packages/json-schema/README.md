...
JSON Schema utilities for NebulaDB - Convert between Zod schemas and JSON Schema formats.
...
```bash
npm install @nebula-db/json-schema zod
```
...
```typescript
import { z } from 'zod';
import { zodToJSONSchema } from '@nebula-db/json-schema';
...
```typescript
import { jsonSchemaToZod } from '@nebula-db/json-schema';
