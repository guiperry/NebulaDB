# @nebula-db/adapter-postgresql

A PostgreSQL adapter for NebulaDB that provides persistent storage using PostgreSQL's JSONB capabilities for efficient document storage and querying.
...
```bash
npm install @nebula-db/core @nebula-db/adapter-postgresql
```
...
```typescript
const adapter = new PostgresqlAdapter({
  connectionString: 'postgresql://user:password@localhost:5432/nebula_db'
});
```
...
const adapter = new PostgresqlAdapter({
  database: 'nebula_db',
  tableName: 'nebula_documents' // Custom table name
});
```
...
Saves NebulaDB data to the PostgreSQL database using transactions for atomicity.
