# @nebula-db/adapter-memorydb

An Amazon MemoryDB adapter for NebulaDB that provides persistent storage using MemoryDB, a Redis-compatible, durable, in-memory database service.
...
```bash
npm install @nebula-db/core @nebula-db/adapter-memorydb
```
...
Saves NebulaDB data to the MemoryDB cluster.
