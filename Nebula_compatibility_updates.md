# NebulaDB Compatibility Updates for Next.js

## Overview

The current NebulaDB package has compatibility issues with Next.js applications, specifically with Server-Side Rendering (SSR) and browser environments. This document outlines the necessary changes to make NebulaDB work seamlessly in Next.js applications.
...
**Location**: `node_modules/@nebula-db/core/dist/index.mjs`
...
**Location**: `database/nebuladb.ts:36`
...
import fs from 'fs';
import type { Database as NebulaDatabase, Collection } from './types';
...
#### File: `packages/nebula-db/src/index.ts`
...
export async function createDatabase(config: any) {
  if (_createDatabase === null) {
    ...
    if (typeof window !== 'undefined') {
      // Browser environment
      const { createBrowserDatabase } = await import('@nebula-db/core/browser-adapter');
      _createDatabase = createBrowserDatabase;
    } else {
      // Node.js environment
      const { createNodeDatabase } = await import('@nebula-db/core/node-adapter');
...
This implementation will make NebulaDB fully compatible with Next.js and other SSR frameworks while maintaining performance and functionality.